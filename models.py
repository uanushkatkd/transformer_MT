...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model:int, h:int, dropout:float)->None:
        ...
        self.d_model=d_model
        self.h=h
        assert d_model%h==0 ,"d_model not divisible by attention heads h"
        self.d_k=d_model//h
    ...
    def attention(query,key,value,mask,dropout: nn.Dropout):
        ...
        attention_scores=attention_scores.softmax(dim=1) # batch,h,sq_l,sq_l
        if dropout is not None:
            attention_scores=dropout(attention_scores)
            # attention score that we're returning will be used for visualization
    ...
    def forward(self,q,k,v,mask):    
        ...
        # batch,sq,dmodel-->batch,sq,h,dk-->batch,h,sq,dk 
        ...
        # batch,h,sq,dk--> batch,sq,h,dk--> batch,sq,d_model  
...
class ProjectionLayer(nn.Module):
    ...
    def forward(self,x):
        # (batch,sq,d_model)-->(batch,sq,vocab)
...
# given all hyperparam build transformer
